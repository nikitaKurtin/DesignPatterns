package gof_exercisers.composite_familiy_tree;
...
class FamilyMember {
    ...
    private void showFamily(int startingGeneration) {
        ...
        for (int i = startingGeneration; i < generation; i++) sb.append('|').append('\t');//add tab for each generation
        sb.append(mFirstName).append(' ').append(mLastName);
        System.out.println(sb);

        for (FamilyMember fm : descendants) fm.showFamily(startingGeneration);
    }
}
